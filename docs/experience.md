...
- F1 是面向用户的执行进度摘要，不是原始协议或模型思考的镜像。对 Kimi 应由结构化事件
  生成稳定的中文阶段名称，原始英文 thinking、命令和工具参数仅作为详情保存；这避免模型
  文案波动、英混显示和隐藏思考泄露。
- 流式归并键必须同时包含聊天、turn、来源类型、代理和可用 item 身份；文本片段不能用
  `strip()` 改写，只能用 `text.strip()` 判断是否全空。这样 status/usage 交错、不同来源
  切换和带 offset 的重放都不会拆句、串流或重复。
- 修改该链路时，按三层验证：事件映射/归并单元与集成测试、wx 焦点和无效重绘自动化测试、
  可用凭据环境下的 opt-in Kimi 实链路冒烟。前两层已在 2026-09-07 验证，不能表述成已做
  真实服务端到端测试。
