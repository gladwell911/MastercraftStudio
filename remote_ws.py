...
class RemoteWebSocketServer:
    ...
    def start(self) -> None:
        ...
        if not self._started.is_set():
            raise RuntimeError("Remote WebSocket server failed to start.")
    ...
    async def _handle_ws_message(self, ws: web.WebSocketResponse, raw: str) -> None:
        try:
            ...
            await ws.send_json({"type": "response", "ok": False, "status": 400, "error": "invalid_json"})
            return
        if not isinstance(payload, dict):
            await ws.send_json({"type": "response", "ok": False, "status": 400, "error": "invalid_payload"})
            return
        ...
        if message_type == "state":
        ...
        elif message_type == "reply_request":
            status, body = await asyncio.to_thread(self.on_reply_request, payload)
        elif message_type == "rename_chat":
            if not callable(self.on_rename_chat):
                await ws.send_json(
            ...
            if not callable(self.on_update_settings):
                await ws.send_json(
                    {"type": "response", "id": request_id, "ok": False, "status": 400, "error": "unsupported_type"}
                )
                return
            status, body = await asyncio.to_thread(self.on_update_settings, payload)
        elif message_type == "history_list":
            if not callable(self.on_history_list):
                await ws.send_json(
                    {"type": "response", "id": request_id, "ok": False, "status": 400, "error": "unsupported_type"}
                )
                return
            status, body = await asyncio.to_thread(self.on_history_list)
        elif message_type == "history_read":
            if not callable(self.on_history_read):
                await ws.send_json(
                    {"type": "response", "id": request_id, "ok": False, "status": 400, "error": "unsupported_type"}
                )
                return
            ...
            return
        await ws.send_json({"type": "response", "id": request_id, "ok": status < 400, "status": status, "body": body})
