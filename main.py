...
class ChatFrame(wx.Frame):
    ...
    def _build_ui(self):
        ...
        self.realtime_call_settings_button = wx.Button(panel, label="语音通话设置")
        row.Add(self.send_button, 0, wx.RIGHT, 8)
        row.Add(self.new_chat_button, 0, wx.RIGHT, 16)
        row.Add(self.realtime_call_settings_button, 0, wx.RIGHT, 16)
        ...
        self.realtime_call_settings_button.MoveAfterInTabOrder(self.new_chat_button)
        self.model_combo.MoveAfterInTabOrder(self.realtime_call_settings_button)
    ...
    def _bind_events(self):
        ...
        if hasattr(self, "realtime_call_settings_button"):
            self.realtime_call_settings_button.Bind(wx.EVT_BUTTON, self._on_open_realtime_call_settings)
        ...
        if hasattr(self, "realtime_call_settings_button"):
            self.realtime_call_settings_button.Bind(wx.EVT_KEY_DOWN, self._on_any_key_down_escape_minimize)
    ...
    def _update_active_answer_row(self, turn_idx: int) -> None:
    ...
    def _answer_markdown_for_output(self, answer_md: str, model: str = "") -> str:
        ...
        if not text:
            return ""
        if self.codex_answer_english_filter_enabled and is_codex_model(model):
    ...
    def _remote_chat_snapshot(self, chat: dict) -> dict:
        if not isinstance(chat, dict):
            return {"chat_id": "", "title": "新聊天", "turns": []}
        ...
        if event.type in {"item_completed", "agent_message_delta", "plan_updated", "diff_updated", "stderr"}:
            ...
            if event.text:
                ...
                if target_idx >= 0 and target_idx < len(self.active_session_turns):
                    ...
                    self._update_active_answer_row(target_idx)
                self._save_state()
                if self.view_mode == "active" and event.type not in {"stderr", "plan_updated", "diff_updated"}:
                    self._render_answer_list()
            return
    ...
    def _build_remote_ws_url(self) -> str:
        ...
        if domain:
            if domain.startswith("ws://") or domain.startswith("wss://"):
                ...
                base = "ws://" + domain[len("http://"):]
            elif domain.startswith("https://"):
                base = "wss://" + domain[len("https://"):]
            else:
                base = "wss://" + domain
        else:
    ...
    def _start_remote_ws_server_if_configured(self) -> None:
        ...
        self.SetStatusText(f"远程 WebSocket 已启动：ws://{host}:{self._remote_ws_server.bound_port}/ws")
    ...
    def _summarize_title(self, turns, api_key: str) -> str:
        ...
        if api_key:
            try:
                ...
                title = ""
        if not title:
            selected = self._title_source_turns(turns)
            last_turn = selected[-1] if selected else {}
            q_last = str(last_turn.get("question") or "").strip()
            a_last = md_to_plain(str(last_turn.get("answer_md") or "")).strip()
            core = q_last or a_last or "新聊天"
    ...
    def _build_title_transcript(self, turns) -> str:
        ...
        if not selected:
            return ""
        lines = []
        lines.append(f"用于命名的聊天片段（共{len(selected)}轮）：")
        for i, turn in enumerate(selected, 1):
            q = str(turn.get("question") or "").strip()
            a = md_to_plain(str(turn.get("answer_md") or "")).strip()
            if q:
    ...
    def _summarize_last_turn_locally(self, turns) -> str:
        ...
        first = selected[0]
        last = selected[-1]
        q_first = str(first.get("question") or "").strip()
        q_last = str(last.get("question") or "").strip()
        a_last = md_to_plain(str(last.get("answer_md") or "")).strip()
        base = " ".join(part for part in [q_first, q_last or a_last] if part).strip() or "新聊天"
        base = base.replace("\r", " ").replace("\n", " ").strip()
        return base[:40] or "新聊天"
