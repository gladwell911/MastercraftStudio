...
class CommonCommandsWriteError(RuntimeError):
    def __init__(self, path: Path, message: str, *, cause: Exception | None = None) -> None:
        super().__init__(f"{message}: {path}")
        self.path = Path(path)
        self.cause = cause
...
class DesktopCommonCommandsStore:
    ...
    def initialize(self) -> None:
        with self._lock:
            ...
            if not self.path.exists():
                self._write_snapshot_unlocked(CommonCommandsSnapshot(revision=0, commands=[]))
    ...
    def _read_snapshot_unlocked(self) -> CommonCommandsSnapshot:
        try:
    ...
    def _write_snapshot_unlocked(self, snapshot: CommonCommandsSnapshot) -> None:
        self.path.parent.mkdir(parents=True, exist_ok=True)
        ...
        try:
            ...
            raise CommonCommandsWriteError(self.path, "failed to write common commands store", cause=exc) from exc
        finally:
            if temp_path.exists():
